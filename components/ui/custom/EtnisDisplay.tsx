...
export function EtnisDisplay({ className }: EtnisDisplayProps) {
    const [data, setData] = React.useState<EtnisData | null>(null);
    ...
    React.useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                setData(result as EtnisData);
            } catch (error) {
                console.error("Failed to load etnis data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
                    ...
                    <CardContent>
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Tidak dapat memuat data etnis penduduk saat ini</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
    ...
    const getEtnisColor = (nama: string) => {
        switch (nama) {
            case "TOTAL":
    ...
    const getEtnisIcon = (nama: string) => {
        switch (nama) {
            case "TOTAL":
                return TrendingUp;
            case "BELUM MENGISI":
    ...
    // Display main etnis groups (excluding TOTAL, JUMLAH, BELUM MENGISI)
    const displayData = data.data.filter(
        (item) => !["TOTAL", "JUMLAH", "BELUM MENGISI"].includes(item.attributes.nama) && item.attributes.jumlah > 0
    );
        ...
        <div className={cn("w-full space-y-6", className)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {displayData.map((card) => {
                    const IconComponent = getEtnisIcon(card.attributes.nama);
                    const colorClass = getEtnisColor(card.attributes.nama);
            ...
            <Card>
                ...
                <CardContent>
                    <div className="space-y-4">
                        {data.data
                            .filter((item) => !["JUMLAH", "TOTAL"].includes(item.attributes.nama))
                            .map((item) => {
                                const IconComponent = getEtnisIcon(item.attributes.nama);
                                return (
                                    <div
                                        key={item.id}
