...
interface BpjsDisplayProps {
    className?: string;
}

const fetchBpjsData = async (): Promise<BpjsData | null> => {
    try {
        const response = await fetch(`/api/statistik/bpjs`);
        if (!response.ok) throw new Error(`Failed to fetch bpjs data: ${response.status}`);
...
export function BpjsDisplay({ className }: BpjsDisplayProps) {
    const [data, setData] = React.useState<BpjsData | null>(null);
    ...
    React.useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                setData(result as BpjsData);
            } catch (error) {
                console.error("Failed to load bpjs- data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
                    ...
                    <CardContent>
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Tidak dapat memuat data BPJS saat ini</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
    ...
    const top3Bpjs = bpjsItems.slice(0, 3);
    const otherBpjs = bpjsItems.slice(3);

    const othersData = otherBpjs.reduce(
        (acc, item) => {
    ...
    const displayCards = [...top3Bpjs];

    if (otherBpjs.length > 0) {
        displayCards.push({
            id: "others",
            type: "others",
            attributes: {
    ...
    } else if (bpjsItems.length < 3) {
        const remainingSlots = 3 - bpjsItems.length;
    ...
    const getBpjsColor = (nama: string, type?: string) => {
    ...
    const getBpjsIcon = (nama: string, type?: string) => {
        ...
        <div className={cn("w-full space-y-6", className)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {displayCards.map((card) => {
                    const IconComponent = getBpjsIcon(card.attributes.nama, card.type);
                    const colorClass = getBpjsColor(card.attributes.nama, card.type);
                    ...
                    return (
                        ...
                        >
                            ...
                            <CardContent className="relative z-10">
                                ...
                                {card.type === "others" ? (
                                    <p className={`text-xs ${textColor}-700`}>BPJS lainnya</p>
                                ) : card.type === "info" ? (
                                    <>
                                        <p className={`text-xs ${textColor}-700`}>total penduduk</p>
                                        <p className={`text-xs ${textColor}-600 mt-1`}>seluruh BPJS </p>
                                    </>
                                ) : card.type === "empty" ? (
