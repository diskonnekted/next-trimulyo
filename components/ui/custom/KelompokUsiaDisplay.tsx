...
interface KelompokUsiaDisplayProps {
    className?: string;
}

const fetchKelompokUsiaData = async (): Promise<KelompokUsiaData | null> => {
    try {
        const response = await fetch(`/api/statistik/kelompok-usia`);
        if (!response.ok) throw new Error(`Failed to fetch kelompok-usia data: ${response.status}`);
        return await response.json();
    } catch (error) {
        console.error("Failed to fetch kelompok-usia data:", error);
        return null;
    }
};

export function KelompokUsiaDisplay({ className }: KelompokUsiaDisplayProps) {
    const [data, setData] = React.useState<KelompokUsiaData | null>(null);
        ...
        const loadData = async () => {
            ...
            try {
                const result = await Promise.race([
                    fetchKelompokUsiaData(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 30000)),
                ]);
                setData(result as KelompokUsiaData);
            } catch (error) {
                console.error("Failed to load kelompok-usia data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
                    ...
                    <CardContent>
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Tidak dapat memuat data kelompok usia saat ini</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
    ...
    const kelompokUsiaItems = data.data
        .filter(
    ...
    const top3Kelompok = kelompokUsiaItems.slice(0, 3);
    const otherKelompok = kelompokUsiaItems.slice(3);

    const othersData = otherKelompok.reduce(
        (acc, item) => {
    ...
    const displayCards = [...top3Kelompok];

    if (otherKelompok.length > 0) {
        displayCards.push({
            id: "others",
            type: "others",
            attributes: {
    ...
    } else if (kelompokUsiaItems.length < 3) {
        const remainingSlots = 3 - kelompokUsiaItems.length;
    ...
    const getKelompokColor = (nama: string, type?: string) => {
    ...
    const getKelompokIcon = (nama: string, type?: string) => {
        ...
        <div className={cn("w-full space-y-6", className)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                {displayCards.map((card) => {
                    const IconComponent = getKelompokIcon(card.attributes.nama, card.type);
                    const colorClass = getKelompokColor(card.attributes.nama, card.type);
                    ...
                    return (
                        ...
                        >
                            ...
                            <CardContent className="relative z-10">
                                ...
                                {card.type === "others" ? (
                                    <p className={`text-xs ${textColor}-700`}>kelompok usia lainnya</p>
                                ) : card.type === "info" ? (
                                    <>
                                        <p className={`text-xs ${textColor}-700`}>total penduduk</p>
                                        <p className={`text-xs ${textColor}-600 mt-1`}>seluruh kelompok usia</p>
                                    </>
                                ) : card.type === "empty" ? (
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        Detail Statistik Kelompok Usia
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {data.data
                            ...
                            .map((item) => {
                                const IconComponent = getKelompokIcon(item.attributes.nama);
                                return (
                                    <div
                                        key={item.id}
