...
import { Heart, Users, UserCheck, Shield, TrendingUp, BarChart3 } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";

interface KontrasepsiKbData {
    data: Array<{
        type: string;
        id: string;
        attributes: {
...
interface KontrasepsiKbDisplayProps {
    className?: string;
}

const fetchKontrasepsiKbData = async (): Promise<KontrasepsiKbData | null> => {
    try {
        const response = await fetch(`/api/statistik/kontrasepsi-kb`);
        if (!response.ok) throw new Error(`Failed to fetch kontrasepsi-kb data: ${response.status}`);
        return await response.json();
    } catch (error) {
        console.error("Failed to fetch kontrasepsi-kb data:", error);
        return null;
    }
};

export function KontrasepsiKbDisplay({ className }: KontrasepsiKbDisplayProps) {
    const [data, setData] = React.useState<KontrasepsiKbData | null>(null);
        ...
        const loadData = async () => {
            ...
            try {
                const result = await Promise.race([
                    fetchKontrasepsiKbData(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 30000)),
                ]);
                setData(result as KontrasepsiKbData);
            } catch (error) {
                console.error("Failed to load kontrasepsi-kb- data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
                    ...
                    <CardContent>
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Tidak dapat memuat data kontrasepsi/KB saat ini</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
    ...
    const kontrasepsiItems = data.data
        .filter(
    ...
    const top3Kontrasepsi = kontrasepsiItems.slice(0, 3);
    const otherKontrasepsi = kontrasepsiItems.slice(3);

    const othersData = otherKontrasepsi.reduce(
        (acc, item) => {
    ...
    const displayCards = [...top3Kontrasepsi];

    if (otherKontrasepsi.length > 0) {
        displayCards.push({
            id: "others",
            type: "others",
            attributes: {
    ...
    } else if (kontrasepsiItems.length < 3) {
        const remainingSlots = 3 - kontrasepsiItems.length;
    ...
    const getKontrasepsiColor = (nama: string, type?: string) => {
    ...
    const getKontrasepsiIcon = (nama: string, type?: string) => {
        ...
        <div className={cn("w-full space-y-6", className)}>
            <Card>
                <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">Data statistik kontrasepsi/KB penduduk Kalurahan</p>
                </CardHeader>
            </Card>

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {displayCards.map((card) => {
                    const IconComponent = getKontrasepsiIcon(card.attributes.nama, card.type);
                    const colorClass = getKontrasepsiColor(card.attributes.nama, card.type);
                    ...
                    return (
                        ...
                        >
                            ...
                            <CardContent className="relative z-10">
                                ...
                                {card.type === "others" ? (
                                    <p className={`text-xs ${textColor}-700`}>kontrasepsi/KB lainnya</p>
                                ) : card.type === "info" ? (
                                    <>
                                        <p className={`text-xs ${textColor}-700`}>total penduduk</p>
                                        <p className={`text-xs ${textColor}-600 mt-1`}>seluruh kontrasepsi/KB</p>
                                    </>
                                ) : card.type === "empty" ? (
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        Detail Statistik Kontrasepsi/KB
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {data.data
                            ...
                            .map((item) => {
                                const IconComponent = getKontrasepsiIcon(item.attributes.nama);
                                return (
                                    <div
                                        key={item.id}
