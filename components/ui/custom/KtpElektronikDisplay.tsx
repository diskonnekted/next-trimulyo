...
interface KtpElektronikDisplayProps {
    className?: string;
}

const fetchKtpElektronikData = async (): Promise<KtpElektronikData | null> => {
    try {
        const response = await fetch(`/api/statistik/ktp-elektronik`);
        if (!response.ok) throw new Error(`Failed to fetch ktp-elektronik data: ${response.status}`);
        return await response.json();
    } catch (error) {
        console.error("Failed to fetch ktp-elektronik data:", error);
        return null;
    }
};

export function KtpElektronikDisplay({ className }: KtpElektronikDisplayProps) {
    const [data, setData] = React.useState<KtpElektronikData | null>(null);
        ...
        const loadData = async () => {
            ...
            try {
                const result = await Promise.race([
                    fetchKtpElektronikData(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 30000)),
                ]);
                setData(result as KtpElektronikData);
            } catch (error) {
                console.error("Failed to load ktp-elektronik data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
                    ...
                    <CardContent>
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Tidak dapat memuat data KTP elektronik saat ini</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
    ...
    const ktpElektronikItems = data.data
        .filter(
    ...
    const top3Ktp = ktpElektronikItems.slice(0, 3);
    const otherKtp = ktpElektronikItems.slice(3);

    const othersData = otherKtp.reduce(
        (acc, item) => {
    ...
    const displayCards = [...top3Ktp];

    if (otherKtp.length > 0) {
        displayCards.push({
            id: "others",
            type: "others",
            attributes: {
    ...
    } else if (ktpElektronikItems.length < 3) {
        const remainingSlots = 3 - ktpElektronikItems.length;
    ...
    const getKtpColor = (nama: string, type?: string) => {
    ...
    const getKtpIcon = (nama: string, type?: string) => {
        if (type === "others") return Users;
        if (type === "info") return BarChart3;
        if (type === "empty") return FileCheck;
        return CreditCard;
    };
        ...
        <div className={cn("w-full space-y-6", className)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {displayCards.map((card) => {
                    const IconComponent = getKtpIcon(card.attributes.nama, card.type);
                    const colorClass = getKtpColor(card.attributes.nama, card.type);
                    ...
                    return (
                        ...
                        >
                            ...
                            <CardContent className="relative z-10">
                                ...
                                {card.type === "others" ? (
                                    <p className={`text-xs ${textColor}-700`}>KTP elektronik lainnya</p>
                                ) : card.type === "info" ? (
                                    <>
                                        <p className={`text-xs ${textColor}-700`}>total penduduk</p>
                                        <p className={`text-xs ${textColor}-600 mt-1`}>seluruh KTP elektronik</p>
                                    </>
                                ) : card.type === "empty" ? (
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        Detail Statistik KTP Elektronik
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {data.data
                            ...
                            .map((item) => {
                                const IconComponent = getKtpIcon(item.attributes.nama);
                                return (
                                    <div
                                        key={item.id}
