...
interface PendidikanDitempuhData {
    data: Array<{
        type: string;
        id: string;
        attributes: {
...
interface PendidikanDitempuhDisplayProps {
    className?: string;
}

const fetchPendidikanDitempuhData = async (): Promise<PendidikanDitempuhData | null> => {
    try {
        const response = await fetch(`/api/statistik/pendidikan-ditempuh`);
        if (!response.ok) throw new Error(`Failed to fetch pendidikan-ditempuh data: ${response.status}`);
        return await response.json();
    } catch (error) {
        console.error("Failed to fetch pendidikan-ditempuh data:", error);
        return null;
    }
};

export function PendidikanDitempuhDisplay({ className }: PendidikanDitempuhDisplayProps) {
    const [data, setData] = React.useState<PendidikanDitempuhData | null>(null);
        ...
        const loadData = async () => {
            ...
            try {
                const result = await Promise.race([
                    fetchPendidikanDitempuhData(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 30000)),
                ]);
                setData(result as PendidikanDitempuhData);
            } catch (error) {
                console.error("Failed to load pendidikan-ditempuh data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
    ...
    const pendidikanItems = data.data
        .filter(
    ...
    const top3Pendidikan = pendidikanItems.slice(0, 3);
    const otherPendidikan = pendidikanItems.slice(3);

    const othersData = otherPendidikan.reduce(
        (acc, item) => {
    ...
    const displayCards = [...top3Pendidikan];

    if (otherPendidikan.length > 0) {
        displayCards.push({
            id: "others",
            type: "others",
            attributes: {
    ...
    } else if (pendidikanItems.length < 3) {
        const remainingSlots = 3 - pendidikanItems.length;
    ...
    const getPendidikanColor = (nama: string, type?: string) => {
    ...
    const getPendidikanIcon = (nama: string, type?: string) => {
        if (type === "others") return Users;
        if (type === "info") return BarChart3;
        if (type === "empty") return GraduationCap;
        return BookOpen;
    };
        ...
        <div className={cn("w-full space-y-6", className)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                {displayCards.map((card) => {
                    const IconComponent = getPendidikanIcon(card.attributes.nama, card.type);
                    const colorClass = getPendidikanColor(card.attributes.nama, card.type);
                    ...
                    return (
                        ...
                        >
                            ...
                            <CardContent className="relative z-10">
                                ...
                                {card.type === "others" ? (
                                    <p className={`text-xs ${textColor}-700`}>pendidikan lainnya</p>
                                ) : card.type === "info" ? (
                                    <>
                                        <p className={`text-xs ${textColor}-700`}>total penduduk</p>
                                        <p className={`text-xs ${textColor}-600 mt-1`}>seluruh pendidikan</p>
                                    </>
                                ) : card.type === "empty" ? (
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        Detail Statistik Pendidikan Ditempuh
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {data.data
                            ...
                            .map((item) => {
                                const IconComponent = getPendidikanIcon(item.attributes.nama);
                                return (
                                    <div
                                        key={item.id}
