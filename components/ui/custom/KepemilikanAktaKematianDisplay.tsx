...
interface KepemilikanAktaKematianData {
    data: Array<{
        type: string;
        id: string;
        attributes: {
...
interface KepemilikanAktaKematianDisplayProps {
    className?: string;
}

const fetchKepemilikanAktaKematianData = async (): Promise<KepemilikanAktaKematianData | null> => {
    try {
        const response = await fetch(`/api/statistik/kepemilikan-akta-kematian`);
        if (!response.ok) throw new Error(`Failed to fetch kepemilikan-akta-kematian data: ${response.status}`);
        return await response.json();
    } catch (error) {
        console.error("Failed to fetch kepemilikan-akta-kematian data:", error);
        return null;
    }
};

export function KepemilikanAktaKematianDisplay({ className }: KepemilikanAktaKematianDisplayProps) {
    const [data, setData] = React.useState<KepemilikanAktaKematianData | null>(null);
        ...
        const loadData = async () => {
            ...
            try {
                const result = await Promise.race([
                    fetchKepemilikanAktaKematianData(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 30000)),
                ]);
                setData(result as KepemilikanAktaKematianData);
            } catch (error) {
                console.error("Failed to load kepemilikan-akta-kematian data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
                    ...
                    <CardContent>
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Tidak dapat memuat data kepemilikan akta kematian saat ini</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
    ...
    const getAktaColor = (nama: string) => {
        switch (nama) {
            case "TOTAL":
    ...
    // Get age group data (excluding TOTAL, JUMLAH, BELUM MENGISI)
    const ageGroupData = data.data.filter(
            ...
            {ageGroupData.length > 0 && (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {ageGroupData.map((card) => {
                            const IconComponent = getAktaIcon(card.attributes.nama);
                            const colorClass = getAktaColor(card.attributes.nama);
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        Detail Kepemilikan Akta Kematian
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {data.data.map((item) => {
                            const IconComponent = getAktaIcon(item.attributes.nama);
                            return (
                                <div
                                    key={item.id}
                                ...
                                >
                                    <div>
                                        ...
                                        {item.attributes.dari !== undefined && item.attributes.sampai !== undefined && (
                                            <p className="text-xs text-muted-foreground">
                                                Umur {item.attributes.dari} - {item.attributes.sampai} tahun
                                            </p>
                                        )}
                                    </div>
