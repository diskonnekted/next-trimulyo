...
import { BarChart3, Home, TrendingUp, Users, Heart } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";

interface PenerimaBantuanKeluargaData {
    data: Array<{
        type: string;
        id: string;
        attributes: {
...
interface PenerimaBantuanKeluargaDisplayProps {
    className?: string;
}

const fetchPenerimaBantuanKeluargaData = async (): Promise<PenerimaBantuanKeluargaData | null> => {
    try {
        const response = await fetch(`/api/statistik/penerima-bantuan-keluarga`);
        if (!response.ok) throw new Error(`Failed to fetch penerima-bantuan-keluarga data: ${response.status}`);
        return await response.json();
    } catch (error) {
        console.error("Failed to fetch penerima-bantuan-keluarga data:", error);
        return null;
    }
};

export function PenerimaBantuanKeluargaDisplay({ className }: PenerimaBantuanKeluargaDisplayProps) {
    const [data, setData] = React.useState<PenerimaBantuanKeluargaData | null>(null);
        ...
        const loadData = async () => {
            ...
            try {
                const result = await Promise.race([
                    fetchPenerimaBantuanKeluargaData(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 30000)),
                ]);
                setData(result as PenerimaBantuanKeluargaData);
            } catch (error) {
                console.error("Failed to load penerima-bantuan-keluarga data:", error);
                setData(null);
            } finally {
                setLoading(false);
            }
                    ...
                    <CardContent>
                        <div className="flex items-center justify-center py-12 text-muted-foreground">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Tidak dapat memuat data penerima bantuan keluarga saat ini</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
    ...
    const penerimaData = data.data.find((item) => item.attributes.nama === "PENERIMA");
    const bukanPenerimaData = data.data.find((item) => item.attributes.nama === "BUKAN PENERIMA");
        ...
        <div className={cn("w-full space-y-6", className)}>
            <Card>
                <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">Data statistik penerima bantuan keluarga Kalurahan</p>
                </CardHeader>
            </Card>
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        <TrendingUp className="h-5 w-5 text-primary" />
                        Detail Statistik Penerima Bantuan Keluarga
                    </CardTitle>
                </CardHeader>
                <CardContent>
